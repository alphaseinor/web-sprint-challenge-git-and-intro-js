...
function addArtist(list, name, years, genre, nationality, bio) {
  return [...list, {
    name,
    years,
    genre,
...
function lotsOfArt(list) {
  return listOfNames(list.filter(artist => artist.paintings > 100))
}
...
function artistInfo(list, name){
  return list.filter(artist => artist.name === name)[0].bio
}
...
function artistByCountry(list, nationality){
  return listOfNames(list.filter(artist => artist.nationality === nationality))
}
